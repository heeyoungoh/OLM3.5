...
<script type="text/javascript">
	...
	function setConfFrmInit(){
		document.all.containerCnf.width = getWidth() + "px"; 
		document.all.containerCnf.height =  getHeight() + "px"; 
	...
	function setInitOneLayout(viewType){
		cntnLayout = new dhtmlXLayoutObject("containerCnf",viewType);	
	}	
	function setInitTwoLayout(){	
		document.getElementById('containerCnf').style.height=getHeight()+'px';
		var scrpt = fnSetScriptMasterDiv();
		if($("#schTreeArea").length>0){$("#schTreeArea").remove();}
		/*$("#divCntnTop").show();*/
		baseLayout=new dhtmlXLayoutObject("containerCnf",layout_2U,dhx_skin_skyblue);
	...
	function setLayoutResize(){	 
		...
		document.cfgFrm.action = "cfgURL.do";
		document.cfgFrm.target="blankFrame";
		document.cfgFrm.submit();
	};
	...
	function fnSaveTreeData(){
		...
		dp.setUpdated(3, true, "updated");
		dp.setUpdated(4, true, "updated"); */
		
		//p_tree.getAllChecked
	...
	function fnSetScriptMasterDiv(){
		...
		divScpt.treeTop=divScpt.treeTop+"<input type='text' class='tree_search' id='schTreeText' style='width:100px;ime-mode:active;' placeholder='Search' value='' text=''/>&nbsp;";
		divScpt.treeTop=divScpt.treeTop+"<a onclick='searchTreeText(\"1\")'><img src='${root}${HTML_IMG_DIR}/btn_icon_search.png'></a>";
		divScpt.treeTop=divScpt.treeTop+"<a onclick='fnSaveTreeData(\"1\")'><img src='${root}${HTML_IMG_DIR}/btn_fileadd.png'></a>";    
		divScpt.treeTop=divScpt.treeTop+"<a onclick='searchTreeText(\"2\")'><img src='${root}${HTML_IMG_DIR}/ico_arrow_left.png'></a> | <a onclick='searchTreeText(\"3\")'><img src='${root}${HTML_IMG_DIR}/ico_arrow_right.png'></a>&nbsp;";
		divScpt.treeTop = divScpt.treeTop+"<a onclick='fnRefreshTree(null,true)'><img src='${root}${HTML_IMG_DIR}/img_refresh.png'></a>";
		divScpt.treeTop=divScpt.treeTop+"</div>";divScpt.cntnTop="";return divScpt;}
...
<span class="btn_pack small icon"><span class="assign"></span><input value="Assign" type="submit" onclick="fnSaveTreeData();"></span>		
		<div id="contentcolumn" >	
		
			<div style="width:100%; height:800px;" class="containerCnf" id="containerCnf" name="containerCnf" scrolling='no'>
