...
var maxIndex = "${maxIndex}";
	...
	function setSubFrame(avg){
		...
		/* My Project */
		/* if(avg == "1") {
			clickOpenClose(1); // submenu가 존재하는 menu닫는 처리
	...
	function setBrdFrame(BoardMgtID, avg, url, boardTypeCD) {
		clickSubMenu(avg); // 클릭한 변경
		var target = "help_content";		
		var data = "s_itemID=${projectID}&BoardMgtID="+BoardMgtID+"&boardTypeCD="+boardTypeCD+"&url="+url+"&screenType=PG";
...
<body id="mainMenu">
    ...
    <div id="foldframe" class="foldframe">
    	<div>
       		<ul class="help_menu">      
                 <!-- Service Request -->
                 <li class="helpstitle plus2"><a onclick="clickOpenClose(2);"><img src="${root}${HTML_IMG_DIR}/icon_pjt_chorder.png"><span class="fontchange">&nbsp;${menu.LN00286}</span></a></li>
                 <li class="helpstitle line minus2" style="display:none;"><a class="on" onclick="clickOpenClose(2);"><img src="${root}${HTML_IMG_DIR}/icon_pjt_chorder.png"><span class="fontchange">&nbsp;${menu.LN00286}</span></a></li> 
             	 	...
             	 	<li class="hlepsub smenu2" style="display:none;"><a id="menuCng8" onclick="setSubFrame(8);">&nbsp;Closed</a></li>
             	 	
                 <!-- Change Management -->
                 <li class="helpstitle plus1"><a onclick="clickOpenClose(1);"><img src="${root}${HTML_IMG_DIR}/icon_pjt_chorder.png"><span class="fontchange">&nbsp;${menu.LN00165}</span></a></li>
                 ...
                 <li class="helpstitle minus3" style="display:none;"><a class="on" onclick="clickOpenClose(3);"><img src="${root}${HTML_IMG_DIR}/icon_pjt_rpt.png"><span class="fontchange">&nbsp;${menu.LN00287}</span></a></li> 
	             	<c:forEach var="list" items="${reportList}" varStatus="status">
						...
						><a id="menuCng${startReportIndex + status.count}" onclick="setRptFrame('${startReportIndex + status.count}','${list.ReportURL}');">&nbsp;${list.Name}</a>
						</li>
					</c:forEach>
				<!-- 1.Board -->
                 <c:forEach var="grpList" items="${boardGrpList}" varStatus="grpStatus">
	                 <li class="helpstitle line plus${grpStatus.count+3}"> 	                 
	                 	<a onclick="clickOpenClose(${grpStatus.count+3});"><img src="${root}${HTML_IMG_DIR}/icon_pjt_brd.png"><span class="fontchange">&nbsp;${grpList.BoardGrpName}</span></a>
	                 </li>
