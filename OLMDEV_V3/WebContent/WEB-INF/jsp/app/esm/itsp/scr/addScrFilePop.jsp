...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		if(scrStatus == 'TSCMP'){
			$(".child_search_head p span").text("  테스트 결과 입력");
			$("#breadcrumb li span").text("Test result");
		}
	});
	...
	function doFileUpload(){
		...
		if (confirm("${CM00001}")) {
			var url  = "saveScrFile.do?scrId="+scrID+"&fltpCode="+fltpCode+"&fileID="+fileID+"&srID=${srID}&srType=${srType}";
			if(scrStatus != null || scrStatus != ''){
				url += "&scrStatus="+scrStatus;
			}
			ajaxSubmit(document.uploadFrm, url,"subFrame");
		}
	}
	
	function selfClose(scrId,scrStatus){
		opener.fnCallBack(scrId,scrStatus);
		self.close();
	}
</script>
		...
		<div id="editArea" style="width:95%;" >
			<table class="tbl_preview mgL10 mgR10" border="0" cellpadding="0" cellspacing="0">
				...
				<tr>
					<th>${menu.LN00076}</th>
					<td>
						<textarea class="txt_file_upload" name="comment" style="width:416px;height:100px;resize: none;"></textarea>
					</td>
				</tr>
				<tr>
					<th>${menu.LN00144}</th>
