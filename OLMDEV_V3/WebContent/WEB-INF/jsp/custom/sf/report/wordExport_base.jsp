					...
					for(int j=0; j<kpiList.size(); j++){
						...
						//==================================================================================================	
				 		...
				 		builder.write(StringUtil.checkNullToBlank(rowCnData.get("toItemIdentifier"))); // ID : 내용
			 	 		...
			 	 		builder.insertCell();
			 	 		builder.getCellFormat().setWidth(275);
			 	 		builder.getCellFormat().getShading().setBackgroundPatternColor(Color.WHITE);
			 	 		builder.getParagraphFormat().setAlignment(ParagraphAlignment.LEFT);
			 	 		builder.getFont().setBold(false);
						...
						builder.insertCell();
			 	 		...
			 	 		String KPIAT00003 = StringUtil.checkNullToBlank(rowCnData.get("AT00003")).replaceAll("&lt;","<").replaceAll("&gt;", ">").replaceAll("&quot;","\"").replace("upload/", logoPath);
				...
				if (ruleSetList.size() > 0) {
					...
					for(int j=0; j<ruleSetList.size(); j++){						
						...
						builder.insertCell();
			 	 		...
			 	 		String RSAT00003 = StringUtil.checkNullToBlank(rowCnData.get("AT00003")).replaceAll("&lt;","<").replaceAll("&gt;", ">").replaceAll("&quot;","\"").replace("upload/", logoPath);
			 	 		if ("1".equals(StringUtil.checkNullToBlank(attrRsHtmlMap.get("AT00003")))) { // type이 HTML인 경우
			 	 			if(StringUtil.checkNullToBlank(rowCnData.get("AT00003")).contains("font-family")){
