			...
			builder.endTable().setAllowAutoFit(false);
	 		...
	 		if ("N".equals(onlyMap)) {
				...
				builder.getRowFormat().setHeight(30.0);
	 	 		...
	 	 		builder.getCellFormat().setVerticalAlignment(CellVerticalAlignment.CENTER);
	 	 		builder.getFont().setBold (true);
	 	 		builder.write(String.valueOf(AttrTypeList.get("AT00003")));  // 목적
					...
					for(int j=0; j<activityList.size(); j++){
						...
						builder.getFont().setName(defaultFont);
			 	 		...
			 	 		for(int k=0; k < itemInfo.size(); k++) {
			 	 			...
			 	 			if(info.containsKey("ValidForm")) validForm = String.valueOf(info.get("ValidFrom")); // 개정일
			 	 		...
			 	 		if("CL01005".equals(classCode)) {
							...
							builder.getRowFormat().setHeight(30.0);
				 	 		...
				 	 		builder.getCellFormat().getShading().setBackgroundPatternColor(Color.WHITE);
				 	 		builder.getParagraphFormat().setAlignment(ParagraphAlignment.LEFT);
				 	 		builder.write(validForm); // 개정일 : 내용
				 	 		//==================================================================================================
				 	 		builder.endRow();
				...
				if (String.valueOf(request.getAttribute("rnrYN")).equals("on")) {
					...
					builder.writeln("[E2E Process] " + StringUtil.checkNullToBlank(data.get("Identifier")) + " " + StringUtil.checkNullToBlank(data.get("TREE_NM")));
				...
				if (String.valueOf(request.getAttribute("cxnYN")).equals("on")) {
					...
					for(int j=0; j<relItemList.size(); j++){
						...
						if(("CN00104").equals(CXNItemTypeCode)) ClassName = "IT SYSTEM";
						
						if("CN00201".equals(CXNItemTypeCode) || "CN00104".equals(CXNItemTypeCode)) {
