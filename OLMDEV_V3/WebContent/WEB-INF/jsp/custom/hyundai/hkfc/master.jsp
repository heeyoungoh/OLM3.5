...
<script type="text/javascript">
    ...
    function fnMainSearch(){
    	if($("#mainSearchValue").val() != "") {
    		$("#mainSearchValue").val($("#mainSearchValue").val().replace(/\<|\>|\"|\'|\%|\;|\&|\+|\-/g,""));
    		var searchMode = $("#searchType option:selected").val()

			if(searchMode == "1") {

		    	var url = "zhkfc_searchItemWFile.do";
		    	var target = "container";
		    	var data = "screenType=main&searchValue="+encodeURIComponent($("#mainSearchValue").val()); 
...
// 		ajaxSubmitNoAdd(document.langFrm, "templateSetting.do?templCode="+tmplCode, "blankFrame");
		...
		if(layerType == "TMPL001" || layerType == "TMPL002"){
			$("#searchUL").show();
		} else {
			$("#searchUL").hide();
		}
...
<%-- 				</c:if>			 --%>
			...
			<ul class="floatR mgR30 mgT3" id="searchUL">
				<li id="searchType">
					<select>
						<option value="1" Selected=Selected>${menu.LN01021}</option>
	...
	<div id="menusection">
		...
		<div class="menu_right" id="menuRight" style="display:block;background:#5E904B;">
			<div class="quick_menu" style="float:left !important">
				<ul>
					<c:forEach var="topMenu" items="${topMenuList}" varStatus="status" >
						<c:if test="${topMenu.MenuID ne 'MN007' }"> <!-- 문의 및 제안 미출력 -->
