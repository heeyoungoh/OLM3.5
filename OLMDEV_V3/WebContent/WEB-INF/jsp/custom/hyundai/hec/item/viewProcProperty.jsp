...
<form name="frontFrm" id="frontFrm" action="#" method="post" onsubmit="return false;"> 
		...
		<div id="itemDiv">
			<div style="height: 22px; padding-top: 10px; width: 100%;">
				<ul>
					<li class="floatR pdR20">
						...
						<input type="checkbox" class="mgR5 chkbox" name="ruleset" checked>Rule Set&nbsp;
						<input type="checkbox" class="mgR5 chkbox" name="model" checked>프로세스 Map&nbsp;
						<input type="checkbox" class="mgR5 chkbox" name="subItem" checked>상세업무절차&nbsp;
						<input type="checkbox" class="mgR5 chkbox" name="sopstp" checked>관련 SOP / STP&nbsp;
			...
			<div id="process" class="mgB30">
				...
				<table class="tbl_preview">
					<tr>	<td class="alignL pdL10">${attrMap.AT00003}</td></tr>
				</table>
			</div>
	...
	<!-- 			</table> -->
			...
			<div id="subItem" class="mgB30">
				...
				<table class="tbl_preview">
					...
					<tr onclick="clickItemEvent(this)" style="cursor: pointer;">
						<td>${no }</td>
						<td class="alignL pdL10"><font style="color: blue; border-bottom: 1px solid blue;">${activityList.ItemName}</font></td>
						<td class="alignL pdL10">${activityList.AT10002}</td>
						<td>
						<c:forEach var="actRoleList" items="${activityList.actRoleList}" varStatus="status">
						<c:if test="${status.index eq 0}">
						<font style="color: blue; border-bottom: 1px solid blue;">
						${actRoleList.fromItemName}
						</font>
						</c:if>
						</c:forEach>
						</td>
						<td>
						<c:forEach var="actSystemList" items="${activityList.actSystemList}" varStatus="status">
						<c:if test="${status.index eq 0}">
						<font style="color: blue; border-bottom: 1px solid blue;">
						${actSystemList.toItemName}
						</font>
						</c:if>
						</c:forEach>
						</td>
			...
			<!-- BIGIN :: 관련 SOP / STP -->
			<div id="sopstp" class="mgB30">
				<p class="cont_title">관련 SOP / STP</p>
				<table class="tbl_preview mgB20">
					...
					<c:set value="1" var="no" />
					<c:forEach var="relItemList" items="${relItemList}" varStatus="status">
					<c:if test="${relItemList.ItemTypeCode eq 'OJ00005' }">
					<tr onclick="clickItemEvent(this)" style="cursor: pointer;">
