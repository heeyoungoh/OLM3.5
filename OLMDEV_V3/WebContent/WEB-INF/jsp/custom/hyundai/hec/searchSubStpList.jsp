...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		//fnSelect('Status', '&Category=ITMSTS', 'dictionyCode', '', 'Select'); // 아이템 상태 조건
	...
	function gridInit(){		
		...
		//p_gridArea.setOnLoadingEnd(setLoadingEnd);
		//START - PAGING
	...
	function setGridData(){
		...
		result.header = "${menu.LN00024},#master_checkbox,표준구분,STP No.,조직, 대분류, 중분류, STP명, 주관팀, 작성자, 개정번호, 개정일, File, ItemID, ClassCode, myItemCnt";
		result.cols = "CHK|ItemTypeName|Identifier|TeamName|L1Name|L2Name|ItemName|OwnerTeamName|Name|CSVersion|ValidFrom|FileIcon|ItemID|ClassCode|myItemCnt";
		result.widths = "30,30,60,120,150,120,120,*,120,80,80,60,60,0,0,0"; // item 검색
		result.sorting = "int,int,str,str,str,str,str,str,str,str,str,str,str,str,str,str";
		result.aligns = "center,center,center,center,center,center,center,left,center,center,center,center,center,center,center,center";
		result.data = "languageID=${sessionScope.loginInfo.sessionCurrLangType}&s_itemID=${s_itemID}&ownerType=${ownerType}&showTOJ=${showTOJ}&markAuthorID=${sessionScope.loginInfo.sessionUserId}"
		...
		/* [기본정보] 조건 선택, 입력값 */
		result.data = result.data + "&ClassCode=CL16004"+ setAllCondition();
		...
		if(ind != 1) {
			...
			if(ind != 12) {
				...
				if(fileCheck.indexOf("blank.gif") < 1) {
					...
					var data = "?itmIfMain=N&s_itemID="+p_gridArea.cells(id, 13).getValue(); 
	...
	function editCheckedAllItems(avg){ 
		...
		for(var i = 0 ; i < checkedRows.length; i++ ){			
			...
			if (items == "") {
				...
				myCnt = p_gridArea.cells(checkedRows[i],15).getValue();
				...
				if(myCnt != "0" ) {
					delMyItem = delMyItem + (delMyItem != "" ? "," : "") + p_gridArea.cells(checkedRows[i], 13).getValue();
				}
				else {
					saveMyItem = saveMyItem + (saveMyItem != "" ? "," : "") + p_gridArea.cells(checkedRows[i], 13).getValue();
				}
			}
		}
		...
		if (items != "") {
			if (avg == "Attribute") {
				...
				if(confirm("Do you really subscibe/unsubscibe this item?")){
					
					var url = "zhec_saveAndDeleteSubScrip.do";
					var data = "delMyItem="+delMyItem+"&saveMyItem="+saveMyItem+"&assignmentType=SUBSCR&accessRight=R&assigned=1&memberID=${sessionScope.loginInfo.sessionUserId}&seq=";
					var target = "blankFrame";
					ajaxPage(url, data, target);					
				...
				$("#s_itemIDs").val(items);
				var url = "subItemInfoReport.do?itemInfoRptUrl=${itemInfoRptUrl}";
