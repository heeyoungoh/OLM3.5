...
<script type="text/javascript">
	...
	function fnSaveWfInstInfo(){		
		if(confirm("${CM00001}")){			
			...
			var url = "zmd_SaveMDWFInstInfo.do";
	...
	function setGridCngtData(avg){
		...
		if(preID != null && preID != "")			
			result.data += "&wfInstanceID="+preID;
		else
			result.data += "&wfInstanceID=${wfInstanceID}";
	...
	function fnGoHanmaruPop(docKey, returnURL) {

		var url = "zmd_goHanmaruWorkflow.do?subject="+ encodeURIComponent($("#subject").val())+"&description="+encodeURIComponent($("#description").val())+"&docKey="+docKey+"&returnURL="+encodeURIComponent(returnURL) ;	
...
<form name="wfInstInfoFrm" id="wfInstInfoFrm" method="post" action="#" onsubmit="return false;">
	...
	<input type="hidden" id="hanmaruParam" name="hanmaruParam" value="" />
	<input type="hidden" id="noViewIDs" name="noViewIDs" value="" />
	<input type="hidden" id="agrCnt" name="agrCnt" value="${agrCnt}" />
	<input type="hidden" id="preWfInstanceID" name="preWfInstanceID" value="${preWfInstanceID}" />
	...
	<div id="objectInfoDiv" class="hidden floatC" style="width:100%;overflow-x:hidden;overflow-y:auto;" >
		...
		<table class="tbl_blue01 mgT10" style="table-layout:fixed;" width="98%" border="0" cellpadding="0" cellspacing="0">
			...
			<tr>
				...
				<td class=" alignL pdL10 last" colspan="5" >
					<input type="text" class="text" id="subject" name="subject" value="${getPJTMap.Subject}" style="width:100%;">
				</td>				
			</tr>	
		</table>
		</div>
	    ...
	    <div class="countList pdT10">
		    ...
		    <li class="alignR"><span class="btn_pack small icon"><span class="add"></span><input value="delete" type="button" onClick="fnSetNoViewIDs();"></span></li>
	 	</div>
		...
		<div class="cop_hdtitle" style="border-bottom:1px solid #ccc">
			<ul>
		        <li class="floatR mgT10">  
					...
					<span class="btn_pack medium icon"><span class="save"></span><input value="Submit" onclick="fnSubmitWfInstInfo()" type="button"></span> 
				</li> 
			</ul>	
		</div>
	</form>	
