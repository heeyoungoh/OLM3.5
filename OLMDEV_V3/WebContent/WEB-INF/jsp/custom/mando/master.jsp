...
<script type="text/javascript">
	...
	jQuery(document).ready(function() {	
		...
		var sessionLoginId = "${sessionScope.loginInfo.sessionCurrLangType}";
		
		if(sessionLoginId == "guest") {
			location.href = "${pageContext.request.contextPath}/mando/loginmandoForm.do";
		}
	...
	function doReturnLanguage(){location.href="mainpage.do?loginIdx=${loginIdx}&screenType=${screenType}&srID=${srID}&mainType=${mainType}&sysCode=${sysCode}&status=${status}&s_templCode=" +tmplCode;}		
	...
	function changeTempl(layerType,layerText, mainUrl, mainScnText, mainUrlFilter, tmplFilter, tmplProjectID, tmplType) { 
		...
		tmplCode=layerType;ajaxSubmitNoAdd(document.langFrm, "templateSetting.do?templCode="+tmplCode, "blankFrame");
		setTopIcon("");
	...
	function setQuickImg(img) {
		for(var i=0; i<$(".topIconImg").length;i++) {
			if("quickImg"+i != img)
				$("#quickImg"+i).attr(("src"), $("#quickImg"+i).attr("src").replace("_on",""));
			else {
				$("#quickImg"+i).attr(("src"), $("#quickImg"+i).attr("src").replace("_on",""));
				$("#quickImg"+i).attr(("src"), $("#quickImg"+i).attr("src").replace(".png","_on.png"));
			}
				
		}
	}
	 function clickMainMenu(menuType, menuName, menuIcon, defDimValueID, menuStyle, scndMenuStyle, menuUrl, boardMgtID, mainType, tmplFilter, projectID, tmplType, quickImg){ 
		//alert("menuType : "+menuType+", menuName :"+menuName+"&tmplFilter="+tmplFilter);
		
		if(boardMgtID != undefined && boardMgtID != "" && boardMgtID.indexOf("Img") > 0) {
			setQuickImg(boardMgtID);
		}
		...
		if( menuType == undefined || menuType == ""){alert("${CM00044}");
		...
		}else if(menuType == "BOARD"){changeCheck=false; url ="boardMgt.do"; if($("#cmbTempl").val()=="LY004"){url="boardAdminMgt.do";} url = url+"?boardMgtID="+boardMgtID;name = "Board"; 
		}else if(menuType == "MYPAGE"){url = "myPage.do?boardMgtID=4&srID=${srID}&mainType=" +mainType+"&myPageTmplFilter=${myPageTmplFilter}"; name = "MYPAGE";
			...
			url = "changeInfoList.do?&mainMenu=0&isMine=N&arcCode=AR000004&menuStyle=csh_bluebooks&screenType=view&tmplFilter=undefined&ProjectID=";
	...
	function fnChangeTmpl(){
		var url = "openChangeTmplPop.do?userID=${sessionScope.loginInfo.sessionUserId}";
		var w = 400;
		var h = 150;
	...
	function fnSetLanguage(avg1,avg2,avg3){
		tmplCode = avg3;

		changeLanguage(avg1,avg2);
	}	
...
<div id="maincontainer">
	...
	<div id="topsection">
		<div class="topL">
		  ...
		  <li class="pdR10 pdL20"><img src="${root}${HTML_IMG_DIR}/icon_admin.png" title="Change Region" class="pdT15" style="cursor:pointer" onClick="fnChangeTmpl();"/></li>
		  <c:if test="${loginIdx == 'BASE'}" > 
		  <li class="pdR10 pdT15" OnClick="fnChangePwd();" alt="USER" titl="USER" style="cursor:pointer"> <font style="color:#0D65B7;font-weight:bold;"><u>${sessionScope.loginInfo.sessionUserNm}</u></font>&nbsp;(${sessionScope.loginInfo.sessionTeamName})</li> 
		  </c:if>
		  <c:if test="${loginIdx != 'BASE'}" > 
		  <li class="pdR10 pdT15" OnClick="fnChangePwd();" alt="USER" titl="USER"> <font style="color:#0D65B7;font-weight:bold;"><u>${sessionScope.loginInfo.sessionUserNm}</u></font>&nbsp;(${sessionScope.loginInfo.sessionTeamName})</li> 
		   ...
		   <li class="pdR10 pdL15 pdT15"> |&nbsp;&nbsp;&nbsp;&nbsp;<img src="${root}${HTML_IMG_DIR}/icon_logout.png" /></li>
		  <li class="logout pdT15"><a href="#" class="logout"> Logout </a></li>         
       </div>
		<div class="floatR" id="MemInfo">
			<ul>	
				<li class="myPage hypen" style="cursor:pointer;" alt="MYPAGE" title="MYPAGE">My Page</li>					
                <li class="pdR10 hypen"><form name="langFrm" id="langFrm" action="#" method="post" onsubmit="return false;">
                    <ul id="nav">
                        <li class="top"><span id="cmbLanguage">${sessionScope.loginInfo.sessionCurrLangNm}</span>
                            ...
                            <c:forEach var="lang" items="${langList}" varStatus="status" >
                                <li onclick="changeLanguage('${lang.CODE}','${lang.NAME}');" style="cursor:pointer">
					...
					<ul id="nav2">
						...
						<ul class="sub2">
							<c:forEach var="templ" items="${templList}" varStatus="status" >							
								<li onclick="changeTempl('${templ.TemplCode}','${templ.TemplText}','${templ.MainURL}','${templ.MainScnText}','${templ.URLFilter}','${templ.TmplFilter}','','${templ.TmplType}');" <c:if test="${templ.Invisible=='1'}" >style="display:none;"</c:if>><a href="#" alt="${templ.TemplText}" id="${templ.TemplCode}" style="cursor:pointer">${templ.TemplText}</a>
				...
				<li class="pdR10 hypen"><form name="cmpnyFrm" id="cmpnyFrm" action="#" method="post" onsubmit="return false;">	
					<ul id="nav3">
						<li class="top"><span id="cmbCampany">${cmpnyText}</span>	
							<c:if test="${isCmpnySelect == 'Y'}">						
								...
								<c:forEach var="cmpny" items="${cmpnyList}" varStatus="status" >
									<li onclick="changeCmpny('${cmpny.CmpnyCode}');"><a href="#" alt="${cmpny.CmpnyText}" id="${cmpny.CmpnyCode}" style="cursor:pointer">${cmpny.CmpnyText}</a>
									</li>
								</c:forEach>
								</ul>
							</c:if>
	...
	<!-- BEGIN ::: MENU -->
	<div id="menusection">
		<div id="menu_wrapper" class="${templ.Style}" >
				...
				<div class="quick_menu" style="float: none !important;">
					<ul>
						<c:forEach var="topMenu" items="${topMenuList}" varStatus="status" >
							<li class="topIcon" alt="${topMenu.MenuID}" title="${topMenu.Name}" onclick="clickMainMenu('${topMenu.MenuID}','${topMenu.Name}','','','','','${topMenu.URL}.do?${topMenu.FILTER}','quickImg${status.index}')"><img class="topIconImg" id="quickImg${status.index}" alt="img${topMenu.MenuID}" src="${root}${HTML_IMG_DIR_SHORTCUT}${topMenu.Icon}" style="cursor:pointer"></img></li>
						</c:forEach>				
					</ul>				
				</div>	
			</div>
		    	...
		    	<c:otherwise>
					<c:forEach var="templ" items="${templList}" varStatus="status">
						<div id="slidemenu0${templ.TemplCode}" class="${templ.Style} topSlideMenu" classNm="${templ.Style}" 
						 ...
						 >
							<ul>
							    <li class="first"><a href="#" class="home" style="padding-left:20px;"><img class="menuicon" src="${root}${HTML_IMG_DIR}/ico_Home.png"></a></li>
