...
<nav class="site-navbar navbar navbar-default navbar-fixed-top navbar-mega" role="navigation">
	<div class="navbar-header">
		<div class="navbar-brand navbar-brand-center site-gridmenu-toggle" data-toggle="gridmenu">
			<li alt="HOME" class="logo"><img src="${root}${HTML_IMG_DIR}/IEP_Logo.png" width="180"></li>
		</div>
	</div>
		    ...
		    <div class="dropdown-menu" role="menu" style="right:5px;">
		    	<c:if test="${sessionScope.loginInfo.sessionMlvl != 'VIEWER' }">
		        	<form name="templFrm" id="templFrm" action="#" method="post" onsubmit="return false;">
		            	<c:forEach var="templ" items="${templList}" varStatus="status" >							
							<li onclick="changeTempl2('${templ.TemplCode}','${templ.TemplText}','${templ.MainURL}','${templ.MainScnText}','${templ.URLFilter}','${templ.TmplFilter}','','${templ.TmplType}');" <c:if test="${templ.Invisible=='1'}" >style="display:none;"</c:if>>
	...
	<div id="menusection">
		    	...
		    	<c:otherwise>
					<c:forEach var="templ" items="${templList}" varStatus="status">
						<div id="slidemenu0${templ.TemplCode}" class="${templ.Style} topSlideMenu" classNm="${templ.Style}" 
						 ...
						 >
							<ul>
		  					    <li alt="HOME" class="logo"><img src="${root}${HTML_IMG_DIR}/logo.png" style="cursor:pointer;padding-left:24px;padding-top:10px;" id="imgHome" alt="HOME"></img></li>
								<c:forEach var="mainMenu" items="${mainMenuList}" varStatus="status" >
									<c:if test="${templ.TemplCode == mainMenu.TemplCode}">
										<c:choose>
							    			<c:when test="${mainMenu.CHILD_MENT_CNT > 0}">
		...
		<div class="menu_right_IEP" id="menuRight" style="display:block;">
			<ul>
				<c:if test="${sessionScope.loginInfo.sessionMlvl != 'VIEWER' }">
				<li class="pdR10" style="float:left;">
				<form name="pjtFrm" id="pjtFrm" action="#" method="post" onsubmit="return false;">	
				<ul id="nav2">
					<li class="top"><span id="pjtTemplate">${projectTxt}</span>							
					<ul class="sub2">
						<c:forEach var="pjt" items="${pjtList}" varStatus="statusys" >							
							<li onclick="changePjt('${pjt.ProjectID}');" <c:if test="${pjt.Deactivated=='1'}" >style="display:none;"</c:if>><a href="#" alt="${pjt.ProjectName}" id="${pjt.ProjectName}">${pjt.ProjectCode} ${pjt.ProjectName}</a>
							</li>								
						</c:forEach>
					</ul>
					</li>						
				...
				<form name="langFrm" id="langFrm" action="#" method="post" onsubmit="return false;"></form>			
				<c:if test="${loginIdx == 'BASE'}" > 
			    <li class="pdR10 hypen" style="float:left;" OnClick="fnChangePwd();" alt="USER" titl="USER"> <font style="color:#0D65B7;font-weight:bold;"><u>${sessionScope.loginInfo.sessionUserNm}</u></font>&nbsp;(${sessionScope.loginInfo.sessionTeamName})</li> 
			    </c:if>
			    <c:if test="${loginIdx != 'BASE'}" > 
			    <li class="pdR10 hypen" style="float:left;" alt="USER" titl="USER"> <font style="color:#0D65B7;font-weight:bold;">
			    
				<form name="templFrm" id="templFrm" action="#" method="post" onsubmit="return false;">	
					<ul id="nav2">
						<li class="top"><span id=""><u>${sessionScope.loginInfo.sessionUserNm}</u></font>&nbsp;(${sessionScope.loginInfo.sessionTeamName})</span>							
						<ul class="sub2">
							<c:forEach var="templ" items="${templList}" varStatus="status" >			
								<c:if test="${sessionScope.loginInfo.sessionAuthLev < 3 && !status.last}">				
								<li onclick="changeTempl2('${templ.TemplCode}','${templ.TemplText}','${templ.MainURL}','${templ.MainScnText}','${templ.URLFilter}','${templ.TmplFilter}','','${templ.TmplType}');" <c:if test="${templ.Invisible=='1'}" >style="display:none;"</c:if>><a href="#" alt="${templ.TemplText}" id="${templ.TemplCode}">${templ.TemplText}</a>
								<script type="text/javascript">isTempLoad['${templ.TemplCode}']=false;</script>
								</li>	
								</c:if>							
							</c:forEach>
			    ...
			    <li class="pdR10 hypen" style="float:left;" alt="QUICK" titl="QUICK"> 
			    
				<form name="templFrm" id="templFrm" action="#" method="post" onsubmit="return false;">	
					<ul id="nav2">
						<li class="top"><span id="">quick menu</span>							
						<ul class="sub2">
							<c:forEach var="templ" items="${templList}" varStatus="status" >						
								<li class="topIcon" alt="${topMenu.MenuID}" title="${topMenu.Name}" onclick="clickMainMenu('${topMenu.MenuID}','${topMenu.Name}','','','','','${topMenu.URL}.do?${topMenu.FILTER}')"><a href="#" alt="${topMenu.Name}">${topMenu.Name}</a></li>
								</li>							
							</c:forEach>
						</ul>
						</li>						
		...
		<div id="contentcolumn" class="nonecontentcolumn">		
			<div class="page" id="container" scrolling='no'>
			 ...
			 <footer class="site-footer">
			 	<div class="site-footer-legal">COPYRIGHT © 2018 DAELIM ALL RIGHTS RESERVED.</div>
