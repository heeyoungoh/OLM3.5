...
(function ($) {
    ...
    $.fn.SumoSelect = function (options) {
    	if(options != undefined){
    		parentWidth = (options.parentWidth != undefined ? options.parentWidth : "");
    	}
        ...
        var settings = $.extend({
            ...
            defaultValues: '' //초기값 세팅 변수, ',' 문자를 Split 하여 데이터 세팅함
        }, options);
        ...
        var ret = this.each(function () {
            ...
            this.sumo = { 
                ...
                createElems: function () {
                    ...
                    var styleCode = (parentWidth != "" ? ('style="width: '+parentWidth+'%;"') : "");
                    O.E.wrap('<div class="SumoSelect"'+styleCode+' tabindex="0">');
                    
                    O.select = O.E.parent();
                    O.caption = $('<span></span>');

                    if(O.selID == "AttrCode") {
                    	O.CaptionBtn = $('<button class="CaptionBtn" style=""><label>Select Attribute Type</label></button>').addClass('SlectBox');
                        O.CaptionCont = $('<div id="displayLabel" style="display:inline;margin-left:10px;"></div>').prepend(O.caption);
                    	O.select.append(O.CaptionBtn);
                    }
                    else if(O.selNAME == "AttrLov[]") {
                        O.CaptionCont = $('<p class="CaptionCont" id="ss'+O.selID+'"><label><i></i></label></p>').addClass('SlectBox').attr('style', O.E.attr('style')).prepend(O.caption);
                    	O.CaptionBtn = $('<p></p>');
                    	O.select.append(O.CaptionBtn);
                    }else {
						var style = "";
						if(O.E.attr('style')) {style = O.E.attr('style').replace("inline-block","block")}
                        O.CaptionCont = $('<p class="CaptionCont"><label><i></i></label></p>').addClass('SlectBox').attr('style', style).prepend(O.caption);
                    	O.CaptionBtn = $('<p></p>');
                    	O.select.append(O.CaptionBtn);
                    }
                ...
                createLi: function (opt,i) {
                    ...
                    if(O.selID == "AttrCode") {
                        li = $('<li id="option' + opt.val() +'" data-val="' + opt.val() + '" onClick="javascript:checkAttrCode(\''+opt.val()+'\' ,\''+opt.text()+'\');"><label>' + opt.text() + '</label></li>');
                    }else if(O.selID == "itemTypeCode") {
                        li = $('<li id="option' + opt.val() +'" data-val="' + opt.val() + '" onClick="javascript:checkItemTypeCode(\''+opt.val()+'\');"><label>' + opt.text() + '</label></li>');
                    }
                   else  if(O.selID == "reqTeamID") { 
                        li = $('<li id="option2' + opt.val() +'" data-val="' + opt.val() + '"><label>' + opt.text() + '</label></li>');
                       O.onOptClick(li);
                    }
                    else  if(O.selID == "reqCompanyID") { 
                        li = $('<li id="option' + opt.val() +'" data-val="' + opt.val() + '" onClick="javascript:fnClickedCompany(\''+opt.val()+'\' ,\''+opt.text()+'\');"><label>' + opt.text() + '</label></li>');
                        O.onOptClick(li);
                    }
					else  if(O.selID == "itemTypeCode") { 
                        li = $('<li id="option' + opt.val() +'" data-val="' + opt.val() + '" onClick="javascript:checkItemTypeCode(\''+opt.val()+'\' ,\''+opt.text()+'\');"><label>' + opt.text() + '</label></li>');
                        O.onOptClick(li);
                    }
                    else {
                        li = $('<li  id="option' + opt.val() +'" data-val="' + opt.val() + '"><label>' + opt.text() + '</label></li>');
                        O.onOptClick(li);
                    }
                    ...
                    if (settings.defaultValues != "") {
                    	var tempValue = settings.defaultValues.split(",");
                    	
                    	for (var i in tempValue) {
                    		if (tempValue[i] == opt.val()){
...
//                                O.hideOpts();
                ...
                basicEvents: function () {
                    ...
                    if(O.selID == "AttrCode") {
	                    O.CaptionBtn.click(function (evt) {
	                        O.E.trigger('click');
