...
// 		ajaxSubmitNoAdd(document.langFrm, "templateSetting.do?templCode="+tmplCode, "blankFrame");
	...
	function changeTempl2(layerType,layerText, mainUrl, mainScnText, mainUrlFilter, tmplFilter, tmplProjectID, tmplType) { 
		...
		url = url + "dTempleCode=" + layerType + "&dTempleText=" + layerText;
	 ...
	 function clickMainMenu(menuType, menuName, menuIcon, defDimValueID, menuStyle, scndMenuStyle, menuUrl, boardMgtID, mainType, tmplFilter, projectID, tmplType){ 
		...
		if( menuType == undefined || menuType == ""){alert("${CM00044}");
			...
			if(menuUrl != undefined){
				...
				}else{	url = menuUrl+"&arcCode="+menuType+"&menuStyle="+menuStyle+"&tmplFilter="+tmplFilter+"&ProjectID="+projectID+"&ProjectName="+projectName; }
			}else{ url = menuUrl; }
