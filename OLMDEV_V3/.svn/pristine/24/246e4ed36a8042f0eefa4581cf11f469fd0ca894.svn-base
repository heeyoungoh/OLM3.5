...
<c:if test="${screenType ne 'model'}"><%@ include file="uiInc.min.jsp" %></c:if>
...
function fnLoadDhtmlxTreeJson(tree, key, cols, value, select, noMsg) {var msg = "${WM00018}";var data = "menuId="+key;data += "&cols=" + cols;data += "&SelectMenuId=" + select;data += "&" + value;try{if(key==null || key == ""){fnLog("ERROR fnLoadDhtmlxTreeJson() data: " + data);fnLog("ERROR fnLoadDhtmlxTreeJson() callstack : " + showCallStack());}} catch(e) {}ajaxTreeLoad("<c:url value='/jsonDhtmlxTreeList.do'/>", data, tree, false, noMsg, msg);}
...
function generateDatePicker(){$(this).css({"ime-mode":"disabled"});$(this).attr("size", "10");$(this).attr("maxlength", "10");$(this).click(function() {doClickCalendar("Y", this);});var root = fnGetRootUrl();var tag = "<img src='${root}${HTML_IMG_DIR}/icon_crd.gif' name=\"Image100\" width=\"16\" height=\"22\" border=\"0\" align=\"absmiddle\" style='cursor:hand' onclick=\"doClickCalendar('N','"+this.id+"');\">";$(tag).appendTo($(this).parent());}
// TODO ::: IE8 대응으로 입시 삭제 
function generateDatePicker(){$(this).css({"ime-mode":"disabled"});$(this).attr("size", "10");$(this).attr("maxlength", "10");$(this).click(function() {doClickCalendar("Y", this);});$(this).blur(function() {closeCalendar();});var root = fnGetRootUrl();var tag = "<img src='${root}${HTML_IMG_DIR}/icon_crd.gif' name=\"Image100\" width=\"16\" height=\"22\" border=\"0\" align=\"absmiddle\" style='cursor:hand' onclick=\"doClickCalendar('N','"+this.id+"');\" onblur=\"closeCalendar();\">";$(tag).appendTo($(this).parent());}
...
function fnSetScriptMasterDiv(){var divScpt = new Object();divScpt.treeTop="<div id='schTreeArea'>";divScpt.treeTop=divScpt.treeTop+"<input type='text' class='tree_search' id='schTreeText' style='width:80px;ime-mode:active;' value='' text=''/>&nbsp;<a onclick='searchTreeText(\"1\")'><img src='${root}/cmm/common/images/btn_icon_search.png'></a> <a onclick='searchTreeText(\"2\")'><img src='${root}/cmm/common/images/icon_arrow_left.png'></a> | <a onclick='searchTreeText(\"3\")'><img src='${root}/cmm/common/images/icon_arrow_right.png'></a>&nbsp;";divScpt.treeTop = divScpt.treeTop+"<a onclick='fnRefreshTree(null,true)'><img src='${root}${HTML_IMG_DIR}/img_refresh.png'></a>";divScpt.treeTop=divScpt.treeTop+"</div>";divScpt.cntnTop="";return divScpt;}
