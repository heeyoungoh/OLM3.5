...
function setOTGridData(){
	...
	result.key = "procConfig_SQL.AddItemReport";
	...
	result.widths = "50,50,100,80,100,180,350"; //8
	...
	if (p_gridArea.getCheckedRows(1).length == 0) {
		...
		if (confirm("${CM00034}")) {
			...
			for ( var i = 0; i < checkedRows.length; i++) {
				if (reportCodes == "") {
					reportCodes = p_gridArea.cells(checkedRows[i], 2).getValue();
				} else {
					reportCodes = reportCodes + "," +p_gridArea.cells(checkedRows[i], 2).getValue();
				}
			}
			
			var url = "pim_AddAllocItemReport.do";
			var target = "ArcFrame";
