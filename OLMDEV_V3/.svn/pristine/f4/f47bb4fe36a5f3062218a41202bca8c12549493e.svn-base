...
// 			}
	...
	function fnTransferItem(){		
		...
		var data = "items=${s_itemID}&authorID=${sessionScope.loginInfo.sessionUserId}&authorName=${sessionScope.loginInfo.sessionUserNm}";
		...
		<div id="functions">
	        ...
	        <c:if test="${itemInfo.ChangeMgt eq '1'}">
		  	...
		  	<c:if test="${srType != null && srType != ''}" >
	   		...
	   		<c:if test="${sessionScope.loginInfo.sessionLogintype == 'editor' and quickCheckOut eq 'Y' and itemInfo.CheckInOption ne '00'}" >
   				<span class="btn_pack small icon"><span class="checkout"></span><input value="Check Out" type="button" onclick="fnQuickCheckOut()"></span>
	   		</c:if>	   		
		   	<c:if test="${itemInfo.ChangeMgt eq '1' && itemInfo.Blocked ne '2' && (sessionScope.loginInfo.sessionUserId eq itemInfo.AuthorID || sessionScope.loginInfo.sessionAuthLev eq '1') && (itemInfo.Status eq 'NEW1' || itemInfo.Status eq 'MOD1' || itemInfo.Status eq 'DEL1') && itemInfo.CheckInOption ne '03A' && itemInfo.CheckInOption ne '00' && accMode ne 'OPS'}" >
		  	...
		  	<c:if test="${itemInfo.ChangeMgt eq '1' && itemInfo.Blocked ne '2' && (sessionScope.loginInfo.sessionUserId eq itemInfo.AuthorID || sessionScope.loginInfo.sessionAuthLev eq '1') && (itemInfo.Status eq 'NEW1' || itemInfo.Status eq 'MOD1' || itemInfo.Status eq 'DEL1') && itemInfo.CheckInOption eq '03A' && itemInfo.CheckInOption ne '00'}" >
	   		...
	   		<c:if test="${itemInfo.ChangeMgt eq '1' && myItem eq 'Y' && itemInfo.CheckInOption eq '00' && (itemInfo.Status eq 'REL' || itemInfo.Status eq 'NEW1' || itemInfo.Status eq 'MOD1') }" >
