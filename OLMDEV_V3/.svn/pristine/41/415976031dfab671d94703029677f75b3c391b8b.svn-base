...
try{
	...
	Map menu = (Map)request.getAttribute("menu");
 	...
 	List ruleSetList = (List)request.getAttribute("ruleSetList");
   ...
   // builder.insertImage(imageFileName, 125, 25);
	...
	if (ruleSetList.size() > 0) {
		for(int i=0; i < ruleSetList.size(); i++){				
			...
			builder.getCellFormat().getShading().setBackgroundPatternColor(Color.WHITE);
			builder.getParagraphFormat().setAlignment(ParagraphAlignment.LEFT);
			builder.write(StringUtil.checkNullToBlank(rowCnData.get("toItemIdentifier"))); // ID
			...
			builder.getParagraphFormat().setAlignment(ParagraphAlignment.LEFT);
			builder.getFont().setBold(false);
			builder.write(StringUtil.checkNullToBlank(rowCnData.get("toItemName"))); // 명칭
			//==================================================================================================	
			builder.endRow(); 
			...
			// 2.ROW : 개요
 	 		...
 	 		builder.getCellFormat().setVerticalAlignment(CellVerticalAlignment.CENTER);
 	 		builder.getFont().setBold (true);
 	 		builder.write(String.valueOf(attrRsNameMap.get("AT00021")));  // 시스템 요구사항
 	 		...
 	 		if ("1".equals(StringUtil.checkNullToBlank(attrRsHtmlMap.get("AT00021")))) { // type이 HTML인 경우
 	 			...
 	 			builder.getFont().setBold(false);
 	 			builder.write(StringUtil.checkNullToBlank(rowCnData.get("AT00021"))); // 개요 : 내용
 	 		}
 	 		//==================================================================================================	
 	 		builder.endRow();
    ...
    String fileName = "Ruleset_Report_" + itemNameOfFileNm + "_" + formatter.format(date) + ".docx";
