...
function saveEditIdName() {
	...
	var id = $("#ID_${i.ItemID}").val();
	var name = $("#Name_${i.ItemID}").val();
	if (id == "") {
		<spring:message code="${sessionScope.loginInfo.sessionCurrLangCode}.WM00107" var="WM00107_1" arguments="${i.RNUM},${menu.LN00015}"/>
		alert("${WM00107_1}");
		return;
	}
	if (name == "") {
		<spring:message code="${sessionScope.loginInfo.sessionCurrLangCode}.WM00107" var="WM00107_2" arguments="${i.RNUM},${menu.LN00028}"/>
		alert("${WM00107_2}");
		return;
	}
	</c:forEach>**/
...
<div id="objectInfoDiv" class="hidden" style="width:100%;">	
	...
	<c:forEach var="i" items="${getList}" varStatus="iStatus">	
		<tr>
			...
			<td class="last">
				<input type="text" id="ID_${i.ItemID}" name="ID_${i.ItemID}" value="${i.Identifier}" class="text" <c:if test="${i.AutoID eq 'Y'}"> disabled </c:if> >
			</td>
			<td class="last">
	...
	<div id="btnSave" style="width:98%;" class="alignBTN" >
