...
<script type="text/javascript">
	...
	jQuery(document).ready(function() {	
		...
		$('.myTeam').click(function(){
			var w = "1200";
			var h = "800";
			var url = "orgMainInfo.do?id=${sessionScope.loginInfo.sessionTeamId}";
	...
	function changeTempl(layerType,layerText, mainUrl, mainScnText, mainUrlFilter, tmplFilter, tmplProjectID, tmplType) { 
	    //alert("layerType:"+layerType+",layerText:"+layerText+",mainUrl:"+mainUrl+",mainScnText:"+mainScnText);
		if(layerType==null||layerType==undefined){
			layerType="${templCode}";
		}else{
			$("#layerTypeHmBtn").val(layerType);
...
// 		ajaxSubmitNoAdd(document.langFrm, "templateSetting.do?templCode="+tmplCode, "blankFrame");
		...
		} else if(mainType == "mySR"){ // SR바로접속 Link My page >> To do list >> Service request (Def = 진행 중)
			clickMainMenu("MYPAGE","MYPAGE", "", "", "", "", "", "", "mySR"); $("#mainType").val("");//home버튼, 로고버튼 이벤트 정상작동위해
		} else if(mainType == "myCSR"){ // CSR바로접속 Link My page >> 2)My page >> To do list >> 변경과제(status = CNG:변경 승인)
			clickMainMenu("MYPAGE","MYPAGE", "", "", "", "", "", "", "myCSR"); $("#mainType").val("");//home버튼, 로고버튼 이벤트 정상작동위해 
		} else if(mainType == "SRDtlView"){
			clickMainMenu("SRREQ","SRREQ", "", "", "", "", "srMgt.do?srType=ITSP&screenType=srRqst&mainType=${mainType}&srID=${srID}");
		}else if(mainType == "srDsk"){ // 외부호출(SR : ServiceDesk)
			clickMainMenu("SRREQ","SRREQ", "", "", "", "", "srMgt.do?srType=ITSP&screenType=srDsk&mainType=${mainType}");
		}else{
	...
	function getTreeSubItems(subItems){
	 ...
	 function clickMainMenu(menuType, menuName, menuIcon, defDimValueID, menuStyle, scndMenuStyle, menuUrl, boardMgtID, mainType, tmplFilter, projectID, tmplType, refreshYN, s_itemID, scrnMode, visitLogYN){ 
		...
		}else if(menuType == "MYPAGE"){url = "myPage.do?noticType=4&srID=${srID}&mainType=" +mainType+"&myPageTmplFilter=${myPageTmplFilter}&status=${status}"  ; name = "MYPAGE";
		...
		}else if(menuType == "AUTHORLOG") {
			url = "itemAuthorLogMgt.do?languageID=${sessionScope.loginInfo.sessionCurrLangType}";
...
<div id="maincontainer">
	...
	<div id="topsection">
		<div class="topL">
		  <li alt="HOME" class="logo"><img src="${root}${HTML_IMG_DIR}/logo_H_PIP.png" style="cursor:pointer" id="imgHome" alt="HOME"></img></li>
		...
		<div class="floatR" id="MemInfo">
			<ul>
				<li class="myTeam pdT5 pdR20" style="cursor:pointer;" alt="MYTEAM" title="MYTEAM">My Team</li>
				<c:if test="${sessionScope.loginInfo.sessionMlvl != 'VIEWER'}">
				<li class="myPage pdT5 pdR20 hypen" style="cursor:pointer;" alt="MYPAGE" title="MYPAGE">My Page</li>
				...
				<li class="help pdT5 pdR20  hypen" alt="HELP" title="HELP" style="cursor:pointer">Help</li>
			</ul>
		</div>		
	</div>
	<!--END::TOP -->	
												...
												<ul>
													<c:forEach var="scnMenu" items="${scnMenuList}" varStatus="status" >
														<c:if test="${mainMenu.MENU_ID == scnMenu.PRNT_MENU_ID}">
															...
															<c:if test="${scnMenu.DimTypeID == '0'}">
																<li onclick="clickMainMenu('${scnMenu.MENU_ID}','${mainMenu.MENU_NM}&nbsp;::&nbsp;${scnMenu.MENU_NM}','${mainMenu.ICON}','','${mainMenu.STYLE}','${scnMenu.STYLE}','${scnMenu.URL}.do?${scnMenu.FILTER}','','','','','','','','','Y')"><a href="#" alt="${scnMenu.MENU_NM}">${scnMenu.MENU_NM}</a></li>
															</c:if>
														</c:if>
													</c:forEach>	
												</ul>
