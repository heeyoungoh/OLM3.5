       ...
       <c:when test="${isView == '0'}">
    	...
    	<c:otherwise>
			...
			<ul onclick="fnDetail('${list.BoardMgtID}','${list.BoardID}')">
			    <li style="width:25%;max-width:595px;">${list.itemName}</li>
			    <li style="width:55%;max-width:595px;">${list.Subject}<c:if test="${list.chkNew == '1'}"><img src="${root}${HTML_IMG_DIR}/new.png"></c:if></li>
			  	<li style="width:10%;" class="alignC">${list.WriteUserNM}</li>
			    <li style="width:15%;" class="alignC">${list.ModDT}</li>
	    	</ul>
	       	</c:forEach>       
		</c:otherwise>
	</c:choose>
