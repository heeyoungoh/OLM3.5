...
<script type="text/javascript">
	...
	$(document).ready(function(){	
		...
		// 초기 표시 화면 크기 조정 
		$("#csrDetailDiv").attr("style","padding: 0 6px 6px 6px; overflow:scroll;overflow-y;overflow-x:hidden;height:"+(setWindowHeight() - 10)+"px;");
	...
	function savePrj() {
		...
		if(confirm("${CM00001}")){
			var url = "saveCSRInfo.do?screenMode=${screenMode}&srID=${srID}&fromSR=${fromSR}&screenType=${screenType}"
	...
	function fnClickedTab(avg, avg2) { 
		...
		if(avg2 == 7){
			...
			var url = "ispList.do";
			var data = "projectID=${getMap.ProjectID}&srType=ISP&srMode=PJT";
			var target="tabFrame";
		...
		var realMenuIndex = "1 2 3 4 5 6 7 8".split(' ');
...
<body>
<div id="csrDiv">
<div id="csrDetailDiv" name="csrDetailDiv" style="padding: 0 6px 6px 6px; height:790px; overflow:scroll;overflow-y;overflow-x:hidden;"> 
	...
	<div id="objectInfoDiv" class="hidden floatC" style="width:100%;overflow-x:hidden;overflow-y:hidden;">
		 	...
		 	<tr>					
		       ...
		       <td class="alignL pdL10 " > 
				...
				<td  class=" alignL pdL10 last">
					<c:if test="${screenMode == 'E'}"> 
						<input type="radio" id="0" name="disclScope" value="0"  <c:if test="${getMap.ItemAccCtrl == '0'}">  checked="checked" </c:if> > <label for="0">All</label> 	
						<input type="radio" id="1" name="disclScope" value="1"  <c:if test="${getMap.ItemAccCtrl == '1'}">  checked="checked" </c:if> > <label for="1">CSR</label> 
					</c:if>
					<c:if test="${screenMode == 'V'}">
						<input type="radio" id="0" name="disclScope" value="0"  <c:if test="${getMap.ItemAccCtrl == '0'}">  checked="checked" </c:if> readOnly> <label for="0">All</label> 	
						<input type="radio" id="1" name="disclScope" value="1"  <c:if test="${getMap.ItemAccCtrl == '1'}">  checked="checked" </c:if> readOnly> <label for="1">CSR</label>
					</c:if>
				</td> 		
			</tr>
		<c:if test="${screenMode == 'V'}"> 
			<tr>
				<!-- CSR Code -->
				<th class="alignL pdL10 viewline ">${menu.LN00129}</th>
		...
		<tr>
		 ...
		 <th class="alignL pdL10 viewline">${menu.LN00290}</th>
	  	...
	  	<tr>
		    <td class="alignR pdR20 last line" bgcolor="#f9f9f9" colspan="6">
      	...
      	<div class="subtabs">
			<ul>
				...
				<li id="pliugt4" style="display:none;"><a href="javascript:fnClickedTab('issus','4');"><span>Issue</span></a></li>
         		<!-- 변경승인 내역 -->
         		<c:if test="${getMap.CurWFInstanceID ne '' && getMap.Status ne 'CSR' && getMap.Status ne 'WTR'}">
...
<!-- 				<li id="pliugt8"><a href="javascript:fnClickedTab('Issue','8');"><span>Issue</span></a></li> -->
			</ul>
		</div>
