...
<script type="text/javascript">
	...
	function doAttachFileV4(){
		...
		openPopup(url+"?"+data,410,410, "Attach File");
	}
	...
	function fnDeleteFileHtmlV4(fileID){			
		var fileName =fileID.textContent;
		//console.log("fnDeleteFileHtml fileID : "+fileID.textContent+" , fileName  :"+fileName); // seq.textContent
	...
	function fnAttacthFileHtmlV4(fileID, fileName){ 	
		display_scripts=$("#tmp_file_items").html(); 
		display_scripts = display_scripts+
			'<div id="'+fileID+'"  class="mm" name="'+fileID+'">'+ fileName+
...
<div>
		...
		<tr>
		    <th class="alignL pdL10" style="height:15px;">${menu.LN00025}(&nbsp;self : <input type="checkbox" id="self" name="self" OnClick="fnCheckRequest()" checked>&nbsp;) </th>
		  	<td class="sline tit last" >
				...
				<input type="hidden" id="requestUserID" name="requestUserID" value="${sessionScope.loginInfo.sessionUserId}" />
				<input type="image" class="image" id="searchRequestBtn" name="searchRequestBtn" style="display:none;" src="${root}${HTML_IMG_DIR}/btn_icon_search.png" onmousedown="searchPopupWf('searchPluralNamePop.do?objId=resultID&objName=resultName&UserLevel=ALL')" value="검색">
			</td>
	...
	<table class="tbl_brd mgT5" style="table-layout:fixed;" width="100%" cellpadding="0" cellspacing="0">
		...
		<tr>
			...
			<td class="sline tit last" colspan="3">
				<input type="text" class="text" id="sharerNames" name="sharerNames" readonly/>			
	...
	<tr> 
		 <th class="alignR pdR20 last" bgcolor="#f9f9f9" colspan="4"  style="vertical-align:middle;" >
			...
			<!--span class="btn_pack medium icon"><span class="upload"></span><input value="AttachV4" type="submit" onclick="doAttachFileV4()"></span-->
