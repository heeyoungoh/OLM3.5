...
#tmpl2_menusection ul > li a.topMenuBG,
#tmpl2_menusection ul > li a:hover{color:#333; background-color:#f2f2f2; cursor:default;font-weight:700;}
...
.search_form{height:30px; margin-top:4px; height:100%; border-radius:15px; border:1px solid #aeaeae; position:relative; width:280px; height:30px;}
.search_form input{border:none;background:none;position:absolute;top:0;left:0;bottom:0;width:calc(100% - 40px);padding-left: 11px;box-sizing: border-box;}
.search_form .btn-search{position:absolute; right:0; bottom:5px; top:5px; width:40px; border:0; background:none; cursor:pointer;}
.search_form .btn-search img{max-width:100%; max-height:100%;}
...
footer .container{display:flex; width:95%; max-width:1920px; min-width:1024px; margin:auto;}
footer .copy_text{margin-left:20px; margin-right:auto;}
...
<script type="text/javascript">
	...
	jQuery(document).ready(function() {	
        ...
        '        <img src=" ${root}${HTML_IMG_DIR}/main/footer_logo.png" alt="현대엔지니어링">'+
        '    </a>'+
        '    <p class="copy_text">Copyrightⓒ Hyundai Engineering co.,Ltd. All rights reserved.</p>'+
        '    <p class="user_info-out">USER : [${sessionScope.loginInfo.sessionLoginId}] ${sessionScope.loginInfo.sessionUserNm}</p>'+
        '</div>';
...
// 		ajaxSubmitNoAdd(document.langFrm, "templateSetting.do?templCode="+tmplCode, "blankFrame");
	...
	function fnLoginForm() {top.window.close();}
    ...
    function fnSearchMain(){
    	...
    	if($("#searchValue").val() != "") {
	    	...
	    	var url = "zhec_SearchItem.do";
	    	var target = "container";
	    	var data = "screenType=main&searchKey=AT00001&searchValue="+encodeURIComponent($("#searchValue").val()); 
...
<div id="maincontainer">
	...
	<header>
		<!-- logo -->
			<div class="container">
		  <li alt="HOME" class="logo"><img src="${root}${HTML_IMG_DIR}/main/logo.png" style="cursor:pointer" id="imgHome" alt="HOME"></img></li>
		  <div class="search_form">
			<input id="searchValue" name="searchValue" placeholder="Search" type="text">	<button class="btn-search" type="button" onClick="fnSearchMain()"><img src="${root}${HTML_IMG_DIR}/main/icon_zoom.png" alt="검색"></button>
			</div>
			<ul class="h_right">
					<li class="hypen ">			
				    ...
				    <li>
				        ...
				        <div class="btn-alert btn-cnt totAlert" alt="TOTALERT" title="TOTALERT">
				            <img src="${root}${HTML_IMG_DIR}/main/icon_alert.png">
				           <c:if test="${boardAlertCnt ne '0' }"><span>${boardAlertCnt}</span></c:if>
				        </div>
				    </li>
