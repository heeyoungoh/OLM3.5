	...
	<c:forEach var="itemTypeList" items="${itemTypeList}" varStatus="i">
		<div style="border-left: 1px solid #ddd; border-right: 1px solid #ddd;">
			<div style="width:100%;height:36px;background: #f2f2f2;padding-top:13px;border-top: 1px solid #ddd;border-bottom: 1px solid #ddd;" >
				<ul>
					<li class="floatL">			
					<span class="fontStyleB15 mgR5"><img src="${root}cmm/common/images/fold.png" class="pdT10 pdB10 pdL10 pdR10 mgL5 mgR5" style="cursor:pointer;" onClick="fnClickItemType('${itemTypeList.ItemTypeCode}',this);" >${itemTypeList.ItemTypeName}</span><span class="fontStyleB12">(${itemTypeList.ItemCount}건)</span>
					</li>
			...
			<c:forEach var="sList" items="${searchList}" varStatus="i">
				...
				<div style="padding: 20px 0 20px;border-bottom:1px solid #ddd;" >
					<ul class="flex justify-between">
						...
						<li class="pdR20" style="flex-basis: 160px;">
					...
					<div class="pdL20 mgT10">
							<c:if test="${!empty sList.AT00003}">${sList.AT00003}<br/></c:if>
							<c:if test="${!empty sList.AT00008}">${sList.AT00008}<br/></c:if>
							<c:if test="${!empty sList.AT00062}">${sList.AT00062}<br/></c:if>
					</div>
					<c:if test="${!empty sList.documentList}">
						<div class="pdL15 mgT20  align-center">
							<input type="text" value="첨부문서" style="height:24px;width:70px;color:#0f80e2;background-color:#e3f0fe;border:0px;border-radius:20px;text-align:center;margin-right: 10px" readOnly >
							<br/>
							<c:forEach var="fileList" items="${sList.documentList}" varStatus="idx"> 								
								<c:if test="${!idx.first}"><br/></c:if><span style="cursor:pointer;color: #808080;" onclick="fileNameClick('${fileList.Seq}','${fileList.Seq}','','${fileList.Seq}','${fileList.Seq}');">
