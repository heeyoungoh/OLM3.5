...
// 		ajaxSubmitNoAdd(document.langFrm, "templateSetting.do?templCode="+tmplCode, "blankFrame");
		...
		}else if(mainType == "myWFDtl"){ // 이메일 결재상세 링크
			clickMainMenu("MYPAGELINK","MYPAGE", "", "", "", "", "", "", mainType);	
	...
	function getTreeSubItems(subItems){
	 ...
	 function clickMainMenu(menuType, menuName, menuIcon, defDimValueID, menuStyle, scndMenuStyle, menuUrl, boardMgtID, mainType, tmplFilter, projectID, tmplType, refreshYN, s_itemID, scrnMode, visitLogYN){ 
		...
		}else if(menuType == "MYPAGELINK"){ // 결재상세 링크 마스터
			var myPageTmplFilter = "${myPageTmplFilter}";
			if(myPageTmplFilter == ""){myPageTmplFilter = "myPageA";}
			if(myPageTmplFilter.indexOf('url') == -1){myPageTmplFilter = "myPageA";}
			url = "myPage.do?srID=${srID}&myPageTmplFilter="+myPageTmplFilter+"&tmplFilter="+tmplFilter+"&mainType="+mainType+"&wfInstanceID=${wfInstanceID}"; name = "MYPAGE";
