...
<script type="text/javascript">
	...
	$(document).ready(function() {	
		...
		$("#ItemTypeCode").SumoSelect({csvDispCount: 3});
	...
	function checkAttrDiv(divClassName,text,ari){
		...
		html += "</select>&nbsp;";
		html += "<input type=\"text\" id=\"searchValue"+divClassName+"\" value=\"\" class=\"stext\" style=\"width:250px;height:28px;margin-left:30px;\">";
	...
	function changeAttrCode2(attrCode, dataType, isComLang) {
		...
		if (dataType == "LOV" || dataType == "MLOV") {
			...
			$("#searchValue"+attrCode).attr('style', 'display:none;width: 250px; height: 28px; margin-left: 30px; ');	
			...
			$("#isLov"+attrCode).val("");
			$("#searchValue"+attrCode).attr('style', 'width: 250px; height: 28px; margin-left: 30px; display: inline;');
	...
	function changeConstraint(avg, avg2) {
		if (avg == "" || avg == "3") {
			if ($("#isLov"+avg2).val() == "Y") {
				$("#searchValue"+avg2).attr('style', 'display:none;width: 250px; height: 28px; margin-left: 30px; ');
				$("#ssAttrLov"+avg2).attr('style', 'width:235px;margin-left:30px;');
			} else {
				$("#searchValue"+avg2).attr('style', 'width: 250px; height: 28px; margin-left: 30px; ');
...
<div>
	...
	<table style="table-layout:fixed;" border="0" cellpadding="0" cellspacing="0" class="tbl_search mgT5"  id="search">
	    ...
	    <tr>
            ...
            <th>${menu.LN00018}</th>
            <td align="left" ><input type="text" id="detailOwnerTeam" name="detailOwnerTeam" value="${searchTeamName}" class="stext" style="width:150px"></td>
            <th>${menu.LN00004}</th>
            <td class="last" align="left"><input type="text" id="detailAuthor" name="detailAuthor" value="${searchAuthorName}" class="stext" style="width:150px"></td>
	    </tr>
