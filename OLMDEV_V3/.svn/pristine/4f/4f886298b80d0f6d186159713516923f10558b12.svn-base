...
<div id="processSRDIV"> 
	...
	<div class="alignBTN">				
		...
		<c:if test="${(srInfoMap.Status ne srInfoMap.FirstSpeCode && srInfoMap.Status ne srInfoMap.LastSpeCode) && elmCodeMap.SUC003 eq 'Y' && sessionScope.loginInfo.sessionAuthLev < 3 }" >
		...
		<c:if test="${srInfoMap.SRNextStatus eq 'SPE008' && elmCodeMap.SUC002 eq 'Y' && createCSR eq 'N' && srInfoMap.srArea3 ne null && srInfoMap.srArea3 ne '0'}" >
		...
		<c:if test="${(srInfoMap.Status eq 'SPE002' && srInfoMap.SRNextStatus eq 'SPE008' && elmCodeMap.SUC002 eq 'Y') || srInfoMap.SRNextStatus eq srInfoMap.LastSpeCode && srInfoMap.SvcCompl eq 'N' && appBtn eq 'Y' && createCSR eq 'N'}" >
		...
		<c:forEach var="list" items="${prLgInfoList}" varStatus="status">
			...
			<c:if test="${list.Link == ''}" ><tr style="height:26px;"></c:if>
				...
				<c:when test="${status.last}">
					<td class="sline tit last alignL " style="font-weight:bold;color:blue;"> * ${list.ActivityName}<c:if test="${list.Link != ''}" ><img class="mgL8" src="${root}cmm/common/images/detail.png" id="popup" style="width:12px; cursor:pointer;" alt="새창열기"></c:if></td>
