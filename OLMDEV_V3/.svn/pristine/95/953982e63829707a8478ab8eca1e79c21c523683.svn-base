...
<script type="text/javascript">
	...
	function setFrame(avg){
		...
		if(avg == 1){
			...
			url = "elmInstAttr.do";
			data += "&elmInstNo=${elmInstNo}&scrnMode=V";
		}else if(avg == 4){
...
<div class="pdL10 pdR10 wrapView pdB10" id="wrapView" style="float:left;" >
	<div class="child_search01 pdT6 pdB5">
		<ul>
			<li class="floatL"><h3 class="floatL"><img src="${root}${HTML_IMG_DIR}/icon_folder_upload_title.png"></img>&nbsp;Project Activity Information</h3></li>
<!-- 			<li class="floatR"> -->
...
<!-- 			</li> -->
	...
	<table style="table-layout: fixed;" width="100%" cellpadding="0" cellspacing="0" border="0" class="tbl_preview">
		...
		<tr>	
			<th>role 명</th>
			<td>${ElmInstInfo.roleName}</td>
			<th>예정시작일</th>
			<td>${ElmInstInfo.SchStartDate}</td>
			<th>Mailing Date</th>
			<td>${ElmInstInfo.AlarmDate}</td>
			<th>상태</th>
			<td class="last">${ElmInstInfo.StatusNM}</td>
		</tr>
		<tr>
			<th>담당자</th>
			<td colspan=7 class="alignL pdL10">${ElmInstInfo.actorName }</td>
		</tr>
		<tr>
			<!-- 첨부 및 관련 문서 --> 
