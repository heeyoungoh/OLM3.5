...
<%@ include file="/WEB-INF/jsp/cmm/menuOnloader_hso.jsp"%>
...
<script type="text/javascript">
	...
	jQuery(document).ready(function() {	
		var lang_style="width:"+"${tmpTextMaxLng}"+"px;";
		fnChangeLangComboStyle(lang_style);
		
		 $('.logo').click(function(){ 	        	
	        $("#cmbTempl").val("${templCode}");changeTempl("${templCode}"); 
	     });
        ...
        $('.home').click(function(){ 
        	...
        	if(layerTypeHmBtn != "TMPL001"){       		
        		$("#cmbTempl").val($("#layerTypeHmBtn").val());
        		changeTempl(layerTypeHmBtn,layerTextHmBtn,mainUrlHmBtn,mainScnTextHmBtn,mainUrlFilterHmBtn); 
        	}else{
        		$("#cmbTempl").val("${templCode}");changeTempl("${templCode}"); 
        	}
	...
	function setInit(){
		...
		changeTempl('${templCode}','${templText}','${mainURL}','${mainScnText}','${mainUrlFilter}');
	...
	function doReturnLanguage(){location.reload();}			
	function changeTempl(layerType,layerText, mainUrl, mainScnText, mainUrlFilter) { 
...
// 		ajaxSubmitNoAdd(document.langFrm, "templateSetting.do?templCode="+tmplCode, "blankFrame");
		...
		if(layerType=="TMPL001"){if (homeUrl==null||homeUrl== undefined){homeUrl = mainUrl;}clickMainMenu("HOME", "HOME", "", "", "", "", homeUrl);}
		else{clickMainMenu("TMPL_CHG_URL", mainScnText, menuIcon, defDimValueID, menuStyle, scndMenuStyle, mainUrl);}
	}
	...
	function logout() {ajaxSubmitNoAdd(document.menuFrm, "<c:url value='/login/logout.do'/>", "blankFrame");}
	...
	function setInitTwoLayout(){	
		...
		olm.menuTree = baseLayout.items[0].attachTree(0);olm.menuTree.setSkin(dhx_skin_skyblue);olm.menuTree.setImagePath("${root}cmm/js/dhtmlx/dhtmlxTree/codebase/imgs/"+treeImgPath+"/");olm.menuTree.attachEvent("onClick",function(id){olm.getMenuUrl(id); return true;});olm.menuTree.enableDragAndDrop(false);olm.menuTree.enableSmartXMLParsing(true);	
	...
	function getTreeSubItems(subItems){
	 ...
	 function clickMainMenu(menuType, menuName, menuIcon, defDimValueID, menuStyle, scndMenuStyle, menuUrl){ 
		var url="";var isChangeLayout=true;var layerType=layout_2E;
		...
		if( menuType == undefined || menuType == ""){alert("${CM00044}");
			...
			//if($("#layerTypeHmBtn").val() != "TMPL001"){
			//	url = $("#mainUrlHmBtn").val()+".do?noticType=1";
		//	}else{
				url = menuUrl+".do?languageID=${sessionScope.loginInfo.sessionCurrLangType}&noticType=1";name = "HOME";
			//}
		}else if(menuType == "BOARD"){changeCheck=false; url ="boardMgt.do"; if($("#cmbTempl").val()=="LY004"){url="boardAdminMgt.do";}url = url+"?noticType=1";name = "게시판";
		}else if(menuType == "MYPAGE"){url = "myPage.do?noticType=4";name = "MYPAGE";
		}else if(menuType == "HELP"){url = "mainHelpMainMenu.do?noticType=1";name = "HELP";
		}else if(menuType == "TMPL_CHG_URL" || menuUrl!=".do?"){url = menuUrl;
		}else{ getCategory(menuType, defDimValueID); 
		}	

		if(url!=undefined && url.length > 0){goMenu(url, menuName, isChangeLayout, layerType,menuType);setInitControl(false, true, menuName, menuIcon);} else{setInitControl(true, true, menuName, menuIcon);}		
	}
	function goMenu(url, name, isChangeLayout, layerType,menuType){
		if( isChangeLayout){setInitOneLayout(layerType);}
		setDisplayTopMenu(menuType);
		if(url=="null"){menuOption=$("#MENU_SELECT").val();creatMenuTab("",url,1);}else if( url.length > 0){ olm.create_tab(100, "", name, url, false);	}	
	}
	function getCategory(avg, defDimValueID){setInitTwoLayout();menuOption = avg;olm.menuTree.getSelectedItemId();olm.menuTree.deleteChildItems(0);var data="";if(defDimValueID != ""){data="defDimValueID="+defDimValueID;}var d=fnSetMenuTreeData(data);fnLoadDhtmlxTreeJson(olm.menuTree, d.key, d.cols, d.data, avg);}
	function setDisplayTopMenu(menuType){var selectedId="slidemenu0"+tmplCode;$('.topSlideMenu').each(function(){var menuId = $(this).attr('id');classNm=$("#"+menuId).attr("classNm");if(menuId==selectedId){$("#"+menuId).attr("style", "display:block");$("#"+menuId).addClass(classNm);if(!isTempLoad[tmplCode]){jqueryslidemenu.buildmenu(menuId, arrowimages);isTempLoad[tmplCode]=true;}}else{$("#"+menuId).attr("style", "display:none;visibility:hidden;");$("#"+menuId).removeClass(classNm);}});}	
...
<div id="maincontainer">
	...
	<div id="topsection">
		...
		<div class="floatR" id="MemInfo">
			<ul>
				...
				<li class="myPage pdT5 pdR20" alt="MYPAGE" title="MYPAGE">MyPage</li>
					...
					<ul id="nav2">
						...
						<ul class="sub2">
							<c:forEach var="templ" items="${templList}" varStatus="status" >
								<li onclick="changeTempl('${templ.TemplCode}','${templ.TemplText}','${templ.MainURL}','${templ.MainScnText}','${templ.URLFilter}');"><a href="#" alt="${templ.TemplText}" id="${templ.TemplCode}">${templ.TemplText}</a>
				...
				<li class="help pdT5 pdR20  hypen" alt="HELP" title="HELP">Help</li>
			</ul>
		</div>		
	</div>
	<!--END::TOP -->	
	...
	<div id="menusection">
		...
		<div class="menu_right" id="menuRight" style="display:block">
