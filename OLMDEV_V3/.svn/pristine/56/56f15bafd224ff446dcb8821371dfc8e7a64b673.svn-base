...
//	console.log("this.cell.childNodes[this.conf.idx.hdr].className :"+this.cell.childNodes[this.conf.idx.hdr].className);
//	console.log("this.conf.idx.hdr :"+this.conf.idx.hdr);
	this.cell.childNodes[this.conf.idx.hdr].className = "dhx_cell_hdr dhx_cell_hdr_hidden";
	this.cell.childNodes[this.conf.idx.cont].className = "dhx_cell_cont_layout_b";
	this._hdrUpdBorder();
	this._mtbUpdBorder();
...
//		t.title = this.i18n.expand;
		t.className = "dhxlayout_arrow dhxlayout_arrow_"+this.conf.mode+this._idd;
		this.cell.childNodes[this.conf.idx.hdr].firstChild.appendChild(t);
		t.onclick = function(e) {
			if (that.conf.collapsed) that.expand(); else that.collapse();
		}
		t = null;
...
dhtmlXLayoutCell.prototype._fitHdr = function() {
	...
	if (this.conf.collapsed == true) {
		...
		h.firstChild.style.width = "17px";
		h.firstChild.firstChild.style.width = "17px";
	} else {
...
dhtmlXLayoutCell.prototype.expand = function(autoExpand) {
	...
	// Smartfactory::ADD
	$("div.dhx_cell_cont_layout").css("top",dhx_cell_hdr_height+2+"px");
	$("div.dhx_cell_cont_layout_b").css("top","0px");
...
dhtmlXLayoutCell.prototype.collapse = function() {
	...
	if (this.conf.mode == "v") {
		...
		console.log("this.conf.size.h :"+this.conf.size.h);
	}
	...
	// Smartfactory::ADD
	$("div.selectTreeArea").css("border-bottom","");
	$("div.dhx_cell_hdr").css("height",this._getHeight()+"px");
