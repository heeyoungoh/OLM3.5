...
public class compareText {
   ...
   */
  public String diff_prettyHtml(List<Diff> diffs, String type) {
    StringBuilder html = new StringBuilder();
    for (Diff aDiff : diffs) {
      String text = aDiff.text.replace("&", "&amp;").replace("<", "&lt;")
          .replace(">", "&gt;").replace("\n", "&para;<br>");
      	
        Operation aDiffOpr = aDiff.operation;
        Operation operationEqual = Operation.EQUAL;
        Operation operationDELETE = Operation.DELETE;
        Operation operationINSERT = Operation.INSERT;
		if (aDiffOpr == operationEqual){
			html.append("<span>").append(text).append("</span>");
			continue;
		}
		
		if(type == "BAS" && aDiffOpr == operationDELETE) {     
			html.append("<del style=\"background:#ffe6e6;\">").append(text)
            .append("</del>");
			continue;
		}
		if(type == "VER" && aDiffOpr == operationINSERT) {
			html.append("<ins style=\"background:#e6ffe6;\">").append(text)
				.append("</ins>");
			continue;
		}
    }
    return html.toString().replace("&amp;", "&").replace("&lt;", "<").replace("&gt;",">").replace("&para;<br>","\n");
  }
