package xbolt.cmm.framework.dhtmlx.xml2pdf;
...
public class PDFWriter {
	...
	private void createPDF() throws Exception {
		...
		*/
		...
		f1 = new Font(pdf, "AdobeMyungjoStd-Medium", CodePage.UNICODE);	//bshyun 20130329
		f1.setSize(fontSize);
	...
	public String pathToDiagram=GlobalVal.MODELING_DIGM_DIR;
	
	public void generateDiagram(String xml, Map imgInfo, HttpServletResponse resp){
		parser = new PDFXMLParser();
		try {
			parser.setXML(xml, resp);			
			createPDF();
	...
	private void printDiagram(Map imgInfo) throws Exception {
		String fileName = pathToDiagram + StringUtil.checkNull(imgInfo.get("imgName")); 
		double percent = (NumberUtil.getDoubleValue(imgInfo.get("imgPercent"),100)/100)*0.55;
		
		//double percent=NumberUtil.getDoubleValue(dPercent/100,1)*0.6;
		System.out.println("percent:"+percent);
		int imgType=0;
		if((GlobalVal.MODELING_DIGM_IMG_TYPE.toUpperCase()).equals("JPG")||(GlobalVal.MODELING_DIGM_IMG_TYPE.toUpperCase()).equals("JEPG")){imgType=ImageType.JPEG;}
		else if((GlobalVal.MODELING_DIGM_IMG_TYPE.toUpperCase()).equals("PNG")){imgType=ImageType.PNG;}
		else{imgType=ImageType.BMP;}
		
		File file = new File(fileName);
		if (file.exists() && file.isFile()) {
	        FileInputStream fis = new FileInputStream(fileName);
			Image im = new Image(pdf, fis, imgType);
			im.setPosition(offsetLeft, offsetTop);
			im.scaleBy(percent);
			im.drawOn(page);
		}else{
