...
public class ExportServlet extends HttpServlet
{
	...
	protected void handleRequest(HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		try {
			...
			String fname = URLDecoder.decode(StringUtil.checkNull(request.getParameter("filename")), "UTF-8");		
	...
	protected String getRequestXml(HttpServletRequest request) throws IOException, UnsupportedEncodingException
	{
		...
		String plain = URLDecoder.decode(StringUtil.checkNull(request.getParameter("plain")), "UTF-8");
		String xml = null;
		...
		if (enc != null && enc.length() > 0){
			...
			xml = plain;//URLDecoder.decode(plain, "UTF-8");
		}
